...
public class AppInstanceManager extends GraphRepository {
	...
	public AppInstanceManager(TitanGraph g) {
		super(g);
	}


	public Vertex getOrCreateAppInstanceVertex(String host, String groupId, String appId) {

	
		TitanGraphQuery q = getGraph().query()
				.has(AppInstance.KEY_VERTEX_TYPE, "AppInstance")
				.has(AppInstance.KEY_HOST, host).has(AppInstance.KEY_GROUP_ID,groupId)
		...
		if (vo.isPresent()) {
			...
			v.setProperty(AppInstance.KEY_VERTEX_TYPE, AppInstance.APP_INSTANCE_TYPE);
			v.setProperty(AppInstance.KEY_HOST, host);
			v.setProperty(AppInstance.KEY_GROUP_ID, groupId);
			v.setProperty(AppInstance.KEY_APP_ID, appId);
			return v;
